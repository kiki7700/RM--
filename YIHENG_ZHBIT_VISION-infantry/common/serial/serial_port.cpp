...
SerialPort::SerialPort(const char* filename, int buadrate)
{
    ...
    success_ = false;
//    serial_mode = NO_INIT;
    fd = open(file_name_, O_RDWR | O_NOCTTY | O_SYNC);// Read/Write access to serial port  对串口的读写访问
    // No terminal will control the process 没有终端控制
    last_fd = fd;
    if(fd == -1)
    {
...
//        NOTICE("port is open " << file_name_,1);
    ...
    struct termios port_settings;               // structure to store the port settings in 结构体存储端口设置
    if(buadrate_==0)
    {
        cfsetispeed(&port_settings, B115200);       // set baud rates  设置波特率（短焦）

        cfsetospeed(&port_settings, B115200);
    }
    else if(buadrate_ == 1)
    {
        cfsetispeed(&port_settings, B921600);       // set baud rates（长焦）
        cfsetospeed(&port_settings, B921600);
    }
    port_settings.c_cflag = (port_settings.c_cflag & ~CSIZE) | CS8;     // 8-bit chars
    // disable IGNBRK for mismatched speed tests; otherwise receive break  对不匹配的速度测试禁用IGNBRK;否则收到破坏
    // as \000 chars
    port_settings.c_iflag &= ~IGNBRK;         // disable break processing 禁用中断处理
    port_settings.c_lflag = 0;                // no signaling chars, no echo,没有信号字符，没有回音，
    // no canonical processing  没有规范处理
    port_settings.c_oflag = 0;                // no remapping, no delays没有重新映射，没有延迟
    port_settings.c_cc[VMIN]  = 0;            // read doesn't block不会阻止读取数据
    port_settings.c_cc[VTIME] = 5;            // 0.5 seconds read timeout0.5秒读取超时
                           ...
                           , int &buff_offset_x, int &buff_offset_y)
{
    ...
    unsigned char read_buffer[7]={0};   /* Buffer to store the data received              */
